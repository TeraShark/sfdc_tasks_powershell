...
while($repeat -eq $true){
    ...
    $again = $(Write-Host "Create another Task? (y/n): " -ForegroundColor Yellow -BackgroundColor DarkGreen -NoNewLine; Read-Host)
    if ($again.ToLower() -ne 'y'){
        $repeat = $false
    }
}
Write-Host "### ===============  Done  =============== ###" -ForegroundColor Yellow
